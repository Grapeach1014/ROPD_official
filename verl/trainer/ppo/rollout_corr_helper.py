...
2. **Memory-Efficient Design**:
   ...
   - Fixed safety bounds (exp(卤20)) for stable exponentiation.
...
3. **Comprehensive Metrics Tracking**:
   ...
   - Off-policy diagnostics (KL divergence, perplexity PPL, log PPL difference, 蠂虏 divergence).
...
- compute_offpolicy_metrics(): called by core functions to calculate off-policy diagnostics
  (KL/PPL/蠂虏) 鈥?no direct external calls needed.
...
# exp(20) 鈮?485 million (upper limit for stable weights), exp(-20) 鈮?2e-9 (lower limit)
SAFETY_BOUND = 20.0
...
def compute_rollout_correction_weights(
    ...
    This function calculates IS weights (蟺_train / 蟺_rollout) using log ratios for numerical stability.
    ...
    if rollout_is == "token":
        # Per-token IS weight: exp(log(蟺_train/蟺_rollout)) with safety clamp
    ...
    # IS weights change the measure, not the objective. See 搂3.2.2 in docs/algo/rollout_corr_math.md
...
def compute_is_metrics(
    ...
    Args:
        rollout_is_weights: Truncated IS weights (蟺_train / 蟺_rollout),
            shape (batch_size, seq_length).
...
def compute_rollout_correction_and_rejection_mask(
    ...
    # Step 1: Compute log ratio (log(蟺_train / 蟺_rollout))
    ...
    # Step 4: Compute off-policy metrics (KL, PPL, 蠂虏, etc.)
...
def compute_offpolicy_metrics(
    ...
    Key metrics:
    - kl: Direct KL divergence estimator KL(蟺_rollout || 蟺_training)
    ...
    - chi2_token: Token-level 蠂虏 divergence E[蟻虏] - 1
    - chi2_seq: Sequence-level 蠂虏 divergence E[(鈭徬乢t)虏] - 1
    ...
    # Formula: exp(-1/|T| * 危 log 蟺_training(y_t|y_<t))
    ...
    if rollout_log_prob is not None:
        # 2a. kl: Direct estimator for KL(蟺_rollout || 蟺_training)
        # This is the standard KL divergence: E[log(蟺_rollout) - log(蟺_training)]
        ...
        # 2b. k3_kl: K3 estimator for KL(蟺_rollout || 蟺_training)
        # More stable for small KL values using: E[exp(log_ratio) - log_ratio - 1]
        # Formula: KL 鈮?E[r - log(r) - 1] where r = 蟺_training/蟺_rollout
        ...
        # 2f. Chi-squared divergence: 蠂虏(蟺_training || 蟺_rollout) = E_渭[蟻虏] - 1
        # where 蟻 = 蟺_training / 蟺_rollout and 渭 = 蟺_rollout (rollout distribution)
        # This measures the variance of importance sampling weights
        # Token-level: E_token[蟻虏] - 1 (averaged over all tokens)
        log_ratio_safe = torch.clamp(log_ratio, min=-SAFETY_BOUND, max=SAFETY_BOUND)
        rho_token = torch.exp(log_ratio_safe)  # 蟻 = 蟺_training / 蟺_rollout (token-level)
        ...
        # Sequence-level: E_seq[(螤 蟻_t)虏] - 1 = E_seq[exp(2 * 危 log 蟻_t)] - 1
        log_ratio_sum = verl_F.masked_sum(log_ratio, response_mask, axis=-1)  # 危 log 蟻_t per sequence
        log_ratio_sum_safe = torch.clamp(log_ratio_sum, min=-SAFETY_BOUND, max=SAFETY_BOUND)
        rho_squared_seq = torch.exp(2.0 * log_ratio_sum_safe)  # (螤 蟻_t)虏
...
def compute_rollout_corr_metrics_from_logprobs(
    ...
    It computes off-policy diagnostic metrics (KL, PPL, 蠂虏) from log probabilities.
