...
ASYNC_REWARD_TENSOR_KEYS: tuple[str, ...] = ("prompts", "responses", "attention_mask", "rm_scores")
ASYNC_REWARD_NON_TENSOR_KEYS: tuple[str, ...] = ("reward_model", "data_source", "extra_info", "uid", "raw_prompt")


def build_async_reward_payload(data: DataProto) -> DataProto:
    from verl import DataProto

    tensor_payload = {
        key: data.batch[key]
        for key in ASYNC_REWARD_TENSOR_KEYS
        if data.batch is not None and key in data.batch.keys()
    }
    non_tensor_payload = {
        key: data.non_tensor_batch[key] for key in ASYNC_REWARD_NON_TENSOR_KEYS if key in data.non_tensor_batch
    }
    meta_info = {}
    if "reward_extra_keys" in data.meta_info:
        meta_info["reward_extra_keys"] = data.meta_info["reward_extra_keys"]
    return DataProto.from_dict(
        tensors=tensor_payload,
        non_tensors=non_tensor_payload,
        meta_info=meta_info,
    )


class PersistentRewardWorker:
    def __init__(self, config=None, tokenizer=None, reward_fn=None):
        ...
        self._init_time_s = time.perf_counter() - init_start

    def compute(self, data: DataProto):
        timing: dict[str, float] = {}
        compute_start = time.perf_counter()
        reward_tensor, reward_extra_infos_dict, reward_control = compute_reward(data, self.reward_fn)
        timing["reward_remote/compute"] = time.perf_counter() - compute_start
        if self._init_time_s is not None:
            timing["reward_remote/init"] = self._init_time_s
            self._init_time_s = None
        return reward_tensor, reward_extra_infos_dict, reward_control, timing
...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager) -> tuple[torch.Tensor, dict[str, Any], dict[str, Any]]:
    ...
    Returns:
        Tuple of reward tensor, extra info dictionary, and reward control dictionary.
    """
    try:
        reward_result = reward_fn(data, return_dict=True)
        reward_tensor = reward_result["reward_tensor"]
        reward_extra_infos_dict = reward_result.get("reward_extra_info", {})
        reward_control = reward_result.get("reward_control", {})
...
PersistentRewardWorkerActor = ray.remote(num_cpus=1)(PersistentRewardWorker)
