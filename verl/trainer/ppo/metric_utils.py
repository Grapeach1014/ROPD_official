    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                if not var_vals:
                    continue
                first_non_none = next((val for val in var_vals if val is not None), None)
                if first_non_none is None:
                    continue
                if not np.isscalar(first_non_none) or isinstance(first_non_none, str):
                    continue
                if any(val is None or not np.isscalar(val) or isinstance(val, str) for val in var_vals):
                    continue
