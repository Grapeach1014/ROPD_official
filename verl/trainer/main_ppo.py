...
def _clone_config_node(node):
    return OmegaConf.create(OmegaConf.to_container(node, resolve=False))


def _reward_kwargs_to_dict(reward_kwargs):
    if reward_kwargs is None:
        return {}
    if OmegaConf.is_config(reward_kwargs):
        reward_kwargs = OmegaConf.to_container(reward_kwargs, resolve=False)
    return dict(reward_kwargs)


def build_val_reward_fn(config, tokenizer):
    val_reward_manager = config.get("val_reward_manager", None)
    val_reward_kwargs = config.reward_model.get("val_reward_kwargs", None)

    if val_reward_manager is None and val_reward_kwargs is None:
        return load_reward_manager(
            config,
            tokenizer,
            num_examine=1,
            **_reward_kwargs_to_dict(config.reward_model.get("reward_kwargs", {})),
        )

    val_config = _clone_config_node(config)
    if val_reward_manager is not None:
        val_config.reward_manager = _clone_config_node(val_reward_manager)

    resolved_val_reward_kwargs = (
        _reward_kwargs_to_dict(val_reward_kwargs)
        if val_reward_kwargs is not None
        else _reward_kwargs_to_dict(config.reward_model.get("reward_kwargs", {}))
    )
    return load_reward_manager(
        val_config,
        tokenizer,
...
def main(config):
    ...
    run_ppo(config, task_runner_class=resolve_task_runner_class(config))
