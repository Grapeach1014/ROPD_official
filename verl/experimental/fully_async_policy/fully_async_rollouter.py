...
async def _await_handle_method(target: Any, method_name: str, *args: Any) -> Any:
    method = getattr(target, method_name)
    remote = getattr(method, "remote", None)
    if remote is not None:
        return await asyncio.wrap_future(remote(*args).future())
    return method(*args)
...
class FullyAsyncRollouter(FullyAsyncRayPPOTrainer):
    ...
    async def set_ropd_async_context(self, raw_group_queue, window_coordinator):
        """Set strict async raw queue/coordinator context for ROPD."""
        async with self.lock:
            self.strict_async_mode = True
            self.raw_group_queue = raw_group_queue
            self.window_coordinator = window_coordinator

    def _attach_window_metadata(self, rollout_sample: RolloutSample, admission: dict[str, Any]) -> RolloutSample:
        param_version = int(admission["param_version"])
        rollout_sample.group_id = admission["group_id"]
        rollout_sample.param_version = param_version
        rollout_sample.param_version_start = [param_version] * len(rollout_sample.full_batch)
        rollout_sample.param_version_end = [param_version] * len(rollout_sample.full_batch)
        rollout_sample.full_batch.non_tensor_batch["param_version"] = [param_version] * len(rollout_sample.full_batch)
        return rollout_sample

    async def _await_window_admission(self, rollout_sample: RolloutSample) -> RolloutSample:
        if not self.strict_async_mode or rollout_sample.group_id:
            return rollout_sample

        timeout_seconds = float(
            self.config.ropd_execution.strict_async.get("admission_block_timeout_seconds", 300)
        )
        deadline = time.monotonic() + timeout_seconds

        while True:
            admission = await _await_handle_method(self.window_coordinator, "request_admission")
            if admission.get("admitted", False):
                return self._attach_window_metadata(rollout_sample, admission)

            if time.monotonic() >= deadline:
                raise TimeoutError(
                    "Timed out while waiting for strict async window admission: "
                    f"{admission.get('reason', 'UNKNOWN')}"
                )

            await asyncio.sleep(0.05)

    async def _put_raw_group_with_backpressure(self, rollout_sample: RolloutSample | None) -> None:
        while self.running:
            success = await _await_handle_method(self.raw_group_queue, "put_nowait", rollout_sample)
            if success:
                return
            await asyncio.sleep(0.05)

        raise RuntimeError("Rollouter stopped before strict async raw group enqueue completed")
    ...
    async def _process_single_sample_streaming(self, rollout_sample: RolloutSample):
        ...
        rollout_sample = await self._await_window_admission(rollout_sample)

        # Calling asynchronous generation methods
        rollout_sample.full_batch.non_tensor_batch["param_version"] = [rollout_sample.param_version] * len(
        ...
        if not is_cancel:
            original_uids = rollout_sample.full_batch.non_tensor_batch.get("uid")
            original_indices = rollout_sample.full_batch.non_tensor_batch.get("index")
            rollout_sample.full_batch = ret
            resolved_uids = rollout_sample.full_batch.non_tensor_batch.get("uid")
            resolved_indices = rollout_sample.full_batch.non_tensor_batch.get("index")
            if original_uids is not None and len(original_uids) == len(rollout_sample.full_batch):
                resolved_uids = original_uids
            elif original_indices is not None and len(original_indices) == len(rollout_sample.full_batch):
                resolved_uids = original_indices
            elif resolved_uids is None and resolved_indices is not None and len(resolved_indices) == len(
                rollout_sample.full_batch
            ):
                resolved_uids = resolved_indices
            elif resolved_uids is None or len(resolved_uids) != len(rollout_sample.full_batch):
                resolved_uids = [f"uid_{rollout_sample.sample_id}"] * len(rollout_sample.full_batch)
            rollout_sample.full_batch.non_tensor_batch["uid"] = np.array(
                [str(uid) for uid in resolved_uids], dtype=object
            )
            rollout_sample.rollout_status = await self.get_statistics()
            rollout_sample.agent_loop_output_list = []

            if self.strict_async_mode:
                await self._put_raw_group_with_backpressure(rollout_sample)
    ...
    async def _streaming_generation_main(self):
        ...
        if self.strict_async_mode:
            await self._put_raw_group_with_backpressure(None)
        else:
