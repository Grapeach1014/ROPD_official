...
from verl.experimental.fully_async_policy.detach_utils import (
    ...
    assemble_batch_from_scored_rollout_samples,
)
from verl.experimental.fully_async_policy.message_queue import MessageQueueClient
from verl.experimental.fully_async_policy.ray_trainer import FullyAsyncRayPPOTrainer
from verl.experimental.fully_async_policy.reward_fn_utils import build_fully_async_reward_fns
...
def _call_handle_method(target: Any, method_name: str, *args: Any) -> Any:
    method = getattr(target, method_name)
    remote = getattr(method, "remote", None)
    if remote is not None:
        return ray.get(remote(*args))
    return method(*args)
...
class FullyAsyncTrainer(FullyAsyncRayPPOTrainer):
    ...
    def __init__(
        ...
        self.config = config
        self.reward_fn, self.val_reward_fn = build_fully_async_reward_fns(config, tokenizer)
        ...
        self.strict_async_mode = False
        self.scored_queue_client = None
        self.window_coordinator = None
    ...
    def set_ropd_async_context(self, scored_queue_client, window_coordinator):
        """Set strict async queue/coordinator context for ROPD."""
        self.strict_async_mode = True
        self.scored_queue_client = scored_queue_client
    ...
    def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        """
        if self.strict_async_mode:
            return self._get_scored_samples_from_queue()
    ...
    def _get_scored_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        print(
            f"[FullyAsyncTrainer] Requesting {self.required_samples} scored samples from strict async queue",
            flush=True,
        )
        ...
        while len(scored_samples) < self.required_samples:
            queue_len = _call_handle_method(self.scored_queue_client, "qsize")
            if queue_len == 0:
                if termination_seen:
                    break
                time.sleep(0.05)
            ...
            sample = _call_handle_method(self.scored_queue_client, "get_nowait")
            queue_len -= 1

            if sample is None:
                print(
                    "[FullyAsyncTrainer] Detected strict async scored queue termination signal. "
                    f"Collected {len(scored_samples)}/{self.required_samples} groups"
                )
        ...
        if not scored_samples or len(scored_samples) < self.required_samples:
            print("[FullyAsyncTrainer] not enough scored samples collected after loop")
            return None, None

        total_wait_time = consumer_end - consumer_start
        batch, reward_extra_info, reward_control = assemble_batch_from_scored_rollout_samples(
            scored_samples,
        ...
        batch.meta_info["fully_async/total_wait_time"] = total_wait_time
        batch.meta_info["strict_async/reward_extra_info"] = reward_extra_info
        batch.meta_info["strict_async/reward_control"] = reward_control

        if self.window_coordinator is not None:
            for group_id in batch.meta_info.get("ropd_group_ids", []):
                _call_handle_method(self.window_coordinator, "mark_consumed", group_id)

        return 0, batch

    def _process_batch_common(self, batch, metrics, timing_raw, local_trigger_step=None):
        if self.strict_async_mode and batch.meta_info.get("ropd_execution_mode") == "strict_async":
            return self._process_strict_async_batch(batch, metrics, timing_raw, local_trigger_step)
        return super()._process_batch_common(batch, metrics, timing_raw, local_trigger_step)

    def _process_strict_async_batch(self, batch, metrics, timing_raw, local_trigger_step=None):
        reward_tensor = batch.batch["token_level_scores"]
        reward_extra_infos_dict = batch.meta_info.pop("strict_async/reward_extra_info", {})
        reward_control = batch.meta_info.pop("strict_async/reward_control", {})

        with marked_timer("old_log_prob", timing_raw, color="blue"):

            def compute_old_log_prob(current_batch):
                old_log_prob = self.actor_rollout_wg.compute_log_prob(current_batch)
                entropys = old_log_prob.batch["entropys"]
                response_masks = current_batch.batch["response_mask"]
                ...
                current_batch = current_batch.union(old_log_prob)
                if "rollout_log_probs" in current_batch.batch.keys():
                    from verl.utils.debug.metrics import calculate_debug_metrics

                    metrics.update(calculate_debug_metrics(current_batch))
            ...
            if async_training and async_training.use_rollout_log_probs:
                ...
                elif "rollout_log_probs" in batch.batch.keys():
        ...
        with marked_timer("adv", timing_raw, color="brown"):
            ...
            if reward_control["quality_gate_stop"]:
                raise RuntimeError(
                    "ROPD strict async reward quality gate requested stop: "
                    f"{reward_control.get('stop_reason', '')}"
                )
            ...
            if reward_extra_infos_dict:
                batch.non_tensor_batch.update({key: np.array(values) for key, values in reward_extra_infos_dict.items()})
    ...
    async def fit(self):
        ...
        self._log_validation_data()
        # 2. perform additional parameter sync / validation if needed
        if await self._maybe_run_final_validation(global_steps=self.global_steps):
            ray.get(self.param_synchronizer.wait_last_valid.remote())
            self._log_validation_data()
    ...
    async def _maybe_run_final_validation(self, global_steps: int | None = None) -> bool:
        """
        Trigger final validation once training data is exhausted.

        In strict async mode, a finished training run may have no next window to advance into.
        Reuse the current parameter version for this final validation-only sync instead of
        forcing a synthetic version bump that would violate the window coordinator state machine.
        """
        should_run_final_validation = (
            self.current_param_version % self.config.rollout.test_freq != 0 or self.local_trigger_step > 1
        )
        if not should_run_final_validation:
            return False

        if self.strict_async_mode and self.local_trigger_step <= 1:
            timing_param_sync = {}
            with marked_timer("timing_s/final_validate_current_param", timing_param_sync):
                ray.get(
                    self.param_synchronizer.sync_weights.remote(
                        self.current_param_version,
                        validate=True,
        ...
        await self._trigger_parameter_sync_after_step(validate=True, global_steps=global_steps)
        return True
