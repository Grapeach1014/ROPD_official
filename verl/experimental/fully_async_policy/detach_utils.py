...
AVERAGED_REWARD_CONTROL_FLOAT_KEYS = frozenset(
    {
        "fallback_rate_initial",
        "fallback_rate_repaired",
...
@dataclass
class ScoredRolloutSample:
    """Rollout sample with judge outputs attached for strict async training."""
...
def assemble_batch_from_rollout_samples(
    ...
    if hasattr(trajectorys_param_versions, "tolist"):
        trajectorys_param_versions = trajectorys_param_versions.tolist()
    else:
        trajectorys_param_versions = list(trajectorys_param_versions)
...
def assemble_batch_from_scored_rollout_samples(
    scored_samples: list[ScoredRolloutSample], tokenizer, config, balance_batch=None
) -> tuple[DataProto, dict[str, list[Any]], dict[str, Any]]:
    """Assemble a trainer-ready batch and merged reward metadata from scored rollout samples."""

    if not scored_samples:
        raise ValueError("Empty scored_samples provided for batch assembly")
    if balance_batch is not None:
        raise NotImplementedError(
            "assemble_batch_from_scored_rollout_samples does not support balance_batch because it can break "
            "token_level_scores and reward metadata alignment"
        )

    batch = assemble_batch_from_rollout_samples(
        [scored_sample.rollout_sample for scored_sample in scored_samples],
        tokenizer=tokenizer,
        config=config,
        balance_batch=balance_batch,
    )

    batch.batch["token_level_scores"] = torch.cat(
        [scored_sample.token_level_scores for scored_sample in scored_samples], dim=0
    )
    batch.meta_info["ropd_execution_mode"] = "strict_async"
    batch.meta_info["ropd_group_ids"] = [
        scored_sample.rollout_sample.group_id for scored_sample in scored_samples
    ]

    merged_reward_extra: dict[str, list[Any]] = defaultdict(list)
    merged_reward_control: dict[str, Any] = {"update_mask": [], "quality_gate_stop": False}

    for scored_sample in scored_samples:
        for key, value in scored_sample.reward_extra_info.items():
            merged_reward_extra[key].extend(list(value))

        merged_reward_control = _merge_reward_control_dict(merged_reward_control, scored_sample.reward_control)

    return batch, dict(merged_reward_extra), merged_reward_control


def _merge_reward_control_dict(base: dict[str, Any], incoming: dict[str, Any]) -> dict[str, Any]:
    merged = deepcopy(base)
    for key, value in incoming.items():
        if key == "update_mask":
            merged.setdefault("update_mask", [])
            merged["update_mask"].extend(list(value))
            continue
        if key == "quality_gate_stop":
            merged["quality_gate_stop"] = bool(merged.get("quality_gate_stop", False) or value)
            continue

        if key in merged:
            merged[key] = _merge_reward_control_value(merged[key], value, key)
        else:
            merged[key] = deepcopy(value)
    return merged


def _merge_reward_control_value(base: Any, incoming: Any, key: str) -> Any:
    if isinstance(base, dict) and isinstance(incoming, dict):
        return _merge_reward_control_dict(base, incoming)
    if isinstance(base, bool) and isinstance(incoming, bool):
        return base or incoming
    if isinstance(base, str) and isinstance(incoming, str):
        ...
        if not incoming or incoming == base:
            return base
        return incoming
    if isinstance(base, (list, tuple)) and isinstance(incoming, (list, tuple)):
        return list(base) + list(incoming)
    if isinstance(base, Integral) and isinstance(incoming, Integral) and not isinstance(base, (bool, np.bool_)):
        return int(base) + int(incoming)
    if isinstance(base, Real) and isinstance(incoming, Real) and not isinstance(base, (bool, np.bool_)):
        if key in AVERAGED_REWARD_CONTROL_FLOAT_KEYS:
            return (float(base) + float(incoming)) / 2.0
        return float(base) + float(incoming)

    raise TypeError(
        f"Unsupported reward_control merge for key '{key}': {type(base).__name__} vs {type(incoming).__name__}"
    )
