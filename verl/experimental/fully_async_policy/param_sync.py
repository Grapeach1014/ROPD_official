...
def _is_ropd_strict_async(config) -> bool:
...
class ParameterSynchronizer:
    ...
    def __init__(self, config, trainer, rollouter, mq, window_coordinator=None):
        self.config = config
        ...
        self.strict_async_mode = _is_ropd_strict_async(config)
    ...
    def sync_weights(self, version, validate=False, global_steps=0, use_trainer_do_validate=False):
        ...
        self._sync_runtime_version(version)
        ...
        self.wait_last_update = self.rollouter.update_param_version.remote(
            self.current_version, validate, global_steps, use_trainer_do_validate
        )
        self.wait_last_resume = self.rollouter.resume.remote(self.wait_last_update)

    def _sync_runtime_version(self, version: int) -> None:
        if self.strict_async_mode:
            self._sync_window_coordinator_version(version)
            return
        if self.mq_client is not None and hasattr(self.mq_client, "update_param_version_sync"):
            self.mq_client.update_param_version_sync(version)

    def _sync_window_coordinator_version(self, version: int) -> None:
        if self.window_coordinator is None:
            return
        if version <= 0:
            return

        snapshot = ray.get(self.window_coordinator.snapshot.remote())
        coordinator_version = int(snapshot["current_param_version"])
        if version == coordinator_version:
            return
        if version < coordinator_version:
            raise RuntimeError(
                f"strict_async param version regressed: coordinator={coordinator_version}, requested={version}"
            )

        ready_for_sync = ray.get(self.window_coordinator.ready_for_sync.remote())
        if not ready_for_sync:
            raise RuntimeError(
                "strict_async sync requested before window ready: "
                f"coordinator={coordinator_version} requested={version} snapshot={snapshot}"
            )

        next_version = ray.get(self.window_coordinator.advance_version.remote())
        if next_version != version:
            raise RuntimeError(
                f"strict_async version mismatch after coordinator.advance_version(): expected={version}, got={next_version}"
            )
