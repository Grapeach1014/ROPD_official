...
try:
    from qwen_vl_utils import fetch_image as _fetch_image
    from qwen_vl_utils import fetch_video as _fetch_video
except ModuleNotFoundError as exc:
    _QWEN_VL_IMPORT_ERROR = exc
    _fetch_image = None
    _fetch_video = None
else:
    _QWEN_VL_IMPORT_ERROR = None


def _require_qwen_vl_utils() -> tuple:
    if _QWEN_VL_IMPORT_ERROR is not None:
        raise ModuleNotFoundError(
            "qwen_vl_utils is required for multimodal SFT data. Install the `geo` extra or `qwen_vl_utils`."
        ) from _QWEN_VL_IMPORT_ERROR

    return _fetch_image, _fetch_video


def process_image(image: dict | Image.Image, image_patch_size: int = 14) -> Image.Image:
    fetch_image, _ = _require_qwen_vl_utils()
...
def process_video(
    ...
    """
    _, fetch_video = _require_qwen_vl_utils()
