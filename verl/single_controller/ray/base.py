...
from ray.util.placement_group import PlacementGroup, get_placement_group, placement_group
...
class RayResourcePool(ResourcePool):
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None, device_name="cuda"):
        ...
        pg_names = [pg_name_prefix + str(idx) for idx in range(len(pg_scheme))]
        existing_pgs = []
        for pg_name in pg_names:
            try:
                existing_pgs.append(get_placement_group(pg_name))
            except ValueError:
                existing_pgs = []
                break

        if existing_pgs:
            self.pgs = sort_placement_group_by_node_ip(existing_pgs)
            return self.pgs

        lifetime = "detached" if self.detached else None

        pgs = [
            placement_group(bundles=bundles, strategy=strategy, name=pg_name, lifetime=lifetime)
            for pg_name, bundles in zip(pg_names, pg_scheme, strict=False)
        ]
