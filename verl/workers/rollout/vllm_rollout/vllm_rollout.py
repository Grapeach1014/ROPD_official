...
def _get_vllm_zmq_tmpdir() -> str:
    tmpdir = (
        os.getenv("VERL_VLLM_ZMQ_TMPDIR")
        or os.getenv("TMPDIR")
        or os.getenv("TEMP")
        or os.getenv("TMP")
...
class vLLMAsyncRollout(BaseRollout):
    ...
    def _init_zeromq(self) -> str:
        ...
        with FileLock(os.path.join(tmpdir, f"verl_vllm_zmq_{username}.lock")):
            context = zmq.asyncio.Context()
            self.socket = context.socket(zmq.REP)
            if socket_type == "ipc":
                pid = os.getpid()
                address = f"ipc://{os.path.join(tmpdir, f'verl_vllm_zmq_{pid}_{username}.ipc')}"
            else:
