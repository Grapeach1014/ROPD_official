...
def _vllm_use_v1_is_explicitly_set() -> bool:
    try:
        return vllm_envs.is_set("VLLM_USE_V1")
    except AttributeError:
        return "VLLM_USE_V1" in os.environ


def _vllm_use_v1_enabled() -> bool:
    if hasattr(vllm_envs, "VLLM_USE_V1"):
        return bool(vllm_envs.VLLM_USE_V1)

    raw_value = os.environ.get("VLLM_USE_V1")
    if raw_value is not None:
        return raw_value.strip().lower() not in {"", "0", "false", "off", "no"}

    # vLLM>=0.12 no longer exposes VLLM_USE_V1 from vllm.envs, and the async
    # server entrypoint already imports from vllm.v1.* in this code path.
    return _VLLM_VERSION >= version.parse("0.12.0")


def _sanitize_sampling_params(sampling_params: dict[str, Any]) -> dict[str, Any]:
    # vLLM expects optional sampling arguments to be omitted instead of being
...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        self.config.max_model_len = resolve_rollout_max_model_len(
            self.config.max_model_len,
            model_max_model_len,
            model_name_or_path=getattr(self.model_config, "path", None),
        )
        logger.info(
            "Resolved rollout max_model_len=%s (model_max_position_embeddings=%s)",
            self.config.max_model_len,
            model_max_model_len,
        )
    ...
    async def run_server(self, args: argparse.Namespace):
        if not _vllm_use_v1_is_explicitly_set():
        ...
        use_v1 = _vllm_use_v1_enabled()
        engine_cls = AsyncLLM if use_v1 else AsyncLLMEngine
        ...
        if "disable_log_requests" in fn_args and hasattr(engine_args, "disable_log_requests"):
        ...
        if self.replica_rank == 0 and self.node_rank == 0:
            engine_kind = "V1" if use_v1 else "V0"
            logger.info(f"Initializing a {engine_kind} LLM engine with config: {vllm_config}")
    ...
    async def run_headless(self, args: argparse.Namespace):
        if CoreEngineProcManager is None:
            raise RuntimeError(
                "Installed vLLM does not provide CoreEngineProcManager; "
                "headless multi-node rollout requires a newer vLLM version."
            )
    ...
    async def wait_for_requests_to_drain(self, timeout_s: float = 300.0, poll_interval_s: float = 0.1):
        native_wait = getattr(self.engine, "wait_for_requests_to_drain", None)
        ...
        if output_processor is None:
            logger.warning(
                "Engine %s does not expose wait_for_requests_to_drain or output_processor; "
                "skipping drain wait before sleep.",
                type(self.engine).__name__,
            )
            return

        has_unfinished_requests = getattr(output_processor, "has_unfinished_requests", None)
        get_num_unfinished_requests = getattr(output_processor, "get_num_unfinished_requests", None)
        if has_unfinished_requests is None:
            logger.warning(
                "Engine %s output processor does not expose unfinished-request state; "
                "skipping drain wait before sleep.",
                type(self.engine).__name__,
            )
            return
        ...
        while has_unfinished_requests():
            if loop.time() >= deadline:
                unfinished = (
                    ...
                    if get_num_unfinished_requests is not None
                    else "unknown"
                )
                raise TimeoutError(
                    f"Timed out waiting for vLLM requests to drain before sleep; "
                    f"unfinished_requests={unfinished}"
                )
