#!/usr/bin/env bash
# Evaluate the base model and every saved 4-GPU FSDP checkpoint on val.parquet.
# This uses the project's official main_ppo + trainer.val_only path.
...
run_candidate() {
  ...
  local args=(
    ...
    --config-name ropd
    --config-name ropd_repro_90m_val
...
# The base run deliberately has checkpoint loading disabled.
run_candidate base disable
for step in 5 10 15 20 25; do
  run_candidate "step_${step}" resume_path "$CKPT_ROOT/global_step_${step}"
done

# Preserve the per-run metric JSONs as the source of truth and write a compact
# machine-readable index for comparison.
find "$OUT_ROOT" -mindepth 2 -maxdepth 2 -name metrics.json -print | sort | \
  jq -R '{path: ., metrics: (input_filename as $ignored | .)}' >/dev/null 2>&1 || true

echo "Validation finished. Per-candidate metrics: $OUT_ROOT/*/metrics.json"
