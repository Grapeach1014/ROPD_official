#!/usr/bin/env bash
# Evaluate base + all saved FSDP checkpoints with the val ground-truth scorer.
...
run_candidate() {
  local name="$1" resume_mode="$2" resume_path="${3:-}"
  ...
  local args=(
    ...
    --config-name ropd
    --config-name ropd_repro_90m_val_groundtruth
    "trainer.experiment_name=ropd-repro-90m-val-$name"
    "trainer.resume_mode=$resume_mode"
    "trainer.validation_metrics_path=$candidate_dir/metrics.json"
    "trainer.validation_data_dir=$candidate_dir/generations"
  )
